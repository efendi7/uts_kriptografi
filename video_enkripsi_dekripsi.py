...
class VideoEncryptionApp:
    def __init__(self, root):
        ...
        self.root.title("Video Encryption Decryption (AES)")
    ...
    def setup_ui(self):
        ...
        self.browse_button = ctk.CTkButton(
            self.frame,
            text="( + ) Browse Video File to Encrypt",
        ...
        self.input_file_label = ctk.CTkLabel(
            self.frame, text="Selected File to Encrypt: None"
        )
        self.input_file_label.grid(row=2, column=0, columnspan=3, padx=6, pady=5)

        self.decryption_browse_button = ctk.CTkButton(
            self.frame,
            text="( + ) Browse Encrypted File to Decrypt",
            command=self.browse_encrypted_file,
            width=626,
            height=70,
        )
        self.decryption_browse_button.grid(
            row=3, column=0, columnspan=3, padx=6, pady=5, sticky="ew"
        )

        self.decryption_file_label = ctk.CTkLabel(
            self.frame, text="Selected Encrypted File: None"
        )
        ...
        self.encryption_button = ctk.CTkButton(
            self.frame, text="Encrypt", command=self.encrypt_file, width=256
        )
        self.encryption_button.grid(row=6, column=0, padx=5, pady=5, sticky="ew")

        self.decryption_button = ctk.CTkButton(
            self.frame, text="Decrypt", command=self.decrypt_file, width=256
        )
        self.decryption_button.grid(row=6, column=1, padx=5, pady=5, sticky="ew")
    ...
    def browse_file(self):
        ...
        if file_path:
            ...
            self.input_file_label.configure(
                text=f"Selected File to Encrypt: {os.path.basename(self.input_file_path)}"
            )
    ...
    def browse_encrypted_file(self):
        ...
        if file_path:
            ...
            self.decryption_file_label.configure(
                text=f"Selected Encrypted File: {os.path.basename(self.decryption_file_path)}"
            )
    ...
    def encrypt_file(self):
        ...
        key = nim_input.encode(
            "utf-8"
        )  # Use the NIM directly as key (you might want to hash it to ensure it's 16 bytes)
        ...
        try:
            ...
            with open(save_path, "wb") as file:
                file.write(
                    binascii.unhexlify(hex_encoded_text.encode())
                )  # Save as binary
    ...
    def decrypt_file(self):
        ...
        if not self.decryption_file_path:
            messagebox.showerror(
                "Error", "Please select an encrypted file to decrypt first!"
            )
            return
